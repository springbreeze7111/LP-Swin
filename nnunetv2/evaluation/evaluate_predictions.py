...
def compute_hd95(mask_ref: np.ndarray, mask_pred: np.ndarray) -> float:
    """
    Compute the 95th percentile of the Hausdorff Distance (HD95) between the reference and prediction masks.

    :param mask_ref: Reference mask (binary array).
    :param mask_pred: Prediction mask (binary array).
    :return: HD95 value.
    """
    ref_points = np.argwhere(mask_ref)  # Extract points from reference mask
    pred_points = np.argwhere(mask_pred)  # Extract points from prediction mask

    if ref_points.size == 0 or pred_points.size == 0:
        # If either mask is empty, return NaN
        return np.nan

    # Compute directed Hausdorff distances
    forward_hd = directed_hausdorff(ref_points, pred_points)[0]
    backward_hd = directed_hausdorff(pred_points, ref_points)[0]
...
def compute_metrics(reference_file: str, prediction_file: str, image_reader_writer: BaseReaderWriter,
    ...
    for r in labels_or_regions:
        ...
        # HD95
        results['metrics'][r]['HD95'] = compute_hd95(mask_ref, mask_pred)
        ...
        if tp + fn > 0:
            ...
            results['metrics'][r]['Recall'] = np.nan
