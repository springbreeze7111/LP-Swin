...
class DC_CE_FocalLoss(nn.Module):
    def __init__(self, soft_dice_kwargs, ce_kwargs, focal_kwargs, weight_ce=1, weight_dice=1, weight_focal=1, ignore_label=None,
                 dice_class=SoftDiceLoss, focal_class=FocalLoss):
        """
        初始化组合损失函数
        ...
        """
        super(DC_CE_FocalLoss, self).__init__()
        if ignore_label is not None:
            ce_kwargs['ignore_index'] = ignore_label
    ...
    def forward(self, net_output: torch.Tensor, target: torch.Tensor):
        ...
        if self.ignore_label is not None:
            assert target.shape[1] == 1, 'ignore label is not implemented for one hot encoded target variables ' \
                                         '(DC_CE_FocalLoss)'
            mask = target != self.ignore_label
            # 将忽略标签替换为 0，因为 Dice Loss 和 Focal Loss 会忽略这些区域
            target_dice = torch.where(mask, target, 0)
            num_fg = mask.sum()
        else:
            target_dice = target
